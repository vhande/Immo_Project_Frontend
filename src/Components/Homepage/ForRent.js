...
function ForRent() {
  ...
  return (
    ...
    <Container className="mb-3 d-flex flex-column justify-content-between justify-content-sm-center align-items-sm-center   flex-md-row flex-lg-row flex-xl-row">
    {result.length !== 0 ? 
    result.slice(0,2).map(item=> 
    <Card className="m-2" style={{"width":"22rem"}}>
      <Card.Img style={{"maxHeight":"200px"}} variant="top" src={`http://localhost:4000${item.file}`} />
      <Card.Body>
